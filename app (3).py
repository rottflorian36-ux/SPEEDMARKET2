...
SCAN_PROMPT = """Tu es un extracteur de donnees. Tu lis une facture et tu extrais CHAQUE LIGNE BRUTE exactement comme elle apparait.

REGLE ABSOLUE: NE RECALCULE RIEN. Copie les chiffres EXACTEMENT comme ils sont sur la facture.

ETAPE 1 — IDENTIFIER LE TYPE DE FACTURE:
- METRO: colonnes = Description | Prix unitaire | VE(colisage) | Qte(colis) | Montant HT | TVA(lettre)
- CARREFOUR/supermarche: colonnes = TVA% | Produit | QTE x P.U. | Montant TTC + lignes "Remise immediate"
- CODIFRANCE: montants en centimes (divise par 100)
- AUTRE: adapte-toi au format

ETAPE 2 — EXTRAIRE CHAQUE LIGNE:

Pour METRO:
- Chaque ligne produit → extrais: desc, prix_unitaire_ht, ve (unites par colis), qte_colis (nombre de colis), montant_ht, tva_code (B/C/D)
- Lignes "COTIS. SECURITE SOCIALE" → ajoute au montant_ht du produit PRECEDENT
- Lignes "Offre Achetez Plus" → soustrais du montant_ht du produit PRECEDENT (1 seule fois si doublon)
- Lignes "3 POUR 2" → extrais comme remise negative
- B=5.5%, C=20%, D=20%
- qty_totale = ve × qte_colis
- IMPORTANT: le Montant sur Metro est DEJA en HT

Pour CARREFOUR/supermarche:
- Chaque ligne produit → extrais: desc, qte, pu_ttc, montant_ttc, tva_rate
- Lignes "Remise immediate -X.XX" → extrais comme ligne SEPAREE: desc="REMISE [produit precedent]", montant_ttc negatif
- NE FUSIONNE PAS les lignes identiques, garde chaque ligne brute
- Les montants sont en TTC

Pour TOUS:
- Extrais aussi les TOTAUX de la facture: total_ht, total_tva, total_ttc, total_avant_remises, total_remises
- Extrais la ventilation TVA si presente
...
FORMAT JSON STRICT (rien d'autre):
{
  ...
  "totaux": {
    ...
    "total_ttc": nombre,
    "total_avant_remises": nombre ou null,
    "total_remises": nombre ou null,
    "ventilation_tva": [
      {"taux": 5.5, "base_ht": nombre, "montant_tva": nombre, "total_ttc": nombre},
      {"taux": 20.0, "base_ht": nombre, "montant_tva": nombre, "total_ttc": nombre}
    ]
  },
  "lignes": [
    {
      ...
      "qte": nombre_total,
      "montant": nombre,
      "type_montant": "ht" ou "ttc",
      "tva": 5.5 ou 20.0 ou 10.0 ou 2.1
    }
  ]
}

REGLES:
- "montant" = le montant EXACT de la ligne tel qu'il apparait (positif ou negatif pour remises)
- "type_montant" = "ht" si le montant est HT (Metro), "ttc" si TTC (Carrefour)
- Pour Metro: qte = VE × nombre_colis. Ex: VE=6, colis=3 → qte=18
- Pour Carrefour: si un produit apparait sur 3 lignes (2x1.75=3.50, 1x1.75=1.75, remise -0.60), extrais LES 3 LIGNES SEPAREMENT
- NE SAUTE AUCUNE LIGNE, NE FUSIONNE RIEN
- Montants negatifs pour les remises/offres
- VERIFIE: la somme de tes montants doit correspondre au total de la facture

IMPORTANT: reponds UNIQUEMENT avec le JSON, rien d'autre. Pas de ```, pas d'explication."""
...
def process_raw_scan(raw_data):
    """
    PYTHON does ALL the math. Takes raw lines from Claude, produces
    aggregated articles with exact totals matching the ticket.
    """
    ...
    # Step 1: Aggregate by product name (sum all lines including negatives)
    agg = OrderedDict()
    for lg in lignes:
        ...
        qte = int(lg.get("qte", 0))
        tva = float(lg.get("tva", 20.0))
        type_m = lg.get("type_montant", "ttc")

        # Skip section headers, totals
        desc_up = desc.upper()
        skip_kw = ["***", "TOTAL", "SOUS-TOTAL", "PALETTE", "PAL.", "PAL ", "CHEP", 
                    "LOGIPAL", "LPR", "CONSIGN", "80X120", "AVOIR", "TRANSPORT",
                    "FRAIS DE PORT", "PRIX AU KG"]
        if any(kw in desc_up for kw in skip_kw):
            continue

        # Clean remise desc
        if desc_up.startswith("REMISE"):
            # Attach to previous product
            if agg:
                last_key = list(agg.keys())[-1]
                agg[last_key]["montant"] += montant
            continue

        if desc not in agg:
            agg[desc] = {"tva": tva, "montant": 0, "qte": 0, "type_montant": type_m}
        agg[desc]["montant"] += montant
        agg[desc]["qte"] += qte

    # Step 2: Build articles with HT/TTC
    articles = []
    for desc, data in agg.items():
        montant = data["montant"]
        qte = max(data["qte"], 1)
        tva_rate = data["tva"]
        type_m = data["type_montant"]

        if type_m == "ht":
            # Metro style: montant is HT
            total_ht = round(montant, 2)
            total_ttc = ttc_from_ht(total_ht, tva_rate)
        else:
            # Carrefour style: montant is TTC
            total_ttc = round(montant, 2)
            total_ht = ht_from_ttc(total_ttc, tva_rate)

        if qte > 0:
            pu_ht = round(total_ht / qte, 2)
            pu_ttc = round(total_ttc / qte, 2)
        else:
            pu_ht = 0
            pu_ttc = 0
...
def gen_pdf(client_name, client_addr, computed_lines, inv_num, inv_date_str, client_siret="", client_tva="", ticket_totaux=None):
    ...
    # Use ticket totals if available, otherwise calculate
    if ticket_totaux and ticket_totaux.get("total_ttc"):
        total_ttc = ticket_totaux["total_ttc"]
        total_tva = ticket_totaux.get("total_tva") or 0
        total_ht = ticket_totaux.get("total_ht") or (total_ttc - total_tva)
    else:
    ...
    if ticket_totaux and ticket_totaux.get("ventilation_tva"):
        for vt in ticket_totaux["ventilation_tva"]:
            ...
            pdf.set_x(xL)
            pdf.cell(30, 5, f"TVA {vt['taux']}% :", 0, 0, "R")
            pdf.cell(30, 5, f"{vt['montant_tva']:.2f} EUR", 0, 1, "R")
    ...
    if uploaded:
                    ...
                    with st.spinner("L'IA extrait les lignes brutes..."):
                        ...
                        if err:
                            ...
                            if articles:
                                ...
                                # Show results
                                calc_ttc = sum(a["total_ttc"] for a in articles)
                                calc_ht = sum(a["total_ht"] for a in articles)
                                ticket_ttc = totaux.get("total_ttc", 0)
                                ticket_ht = totaux.get("total_ht", 0)
                                ...
                                if ticket_ttc:
                                    ...
                                    if diff_ttc < 0.10:
                                        st.success(f"✅ Total TTC ticket: {ticket_ttc:.2f}€ — Calcul: {calc_ttc:.2f}€ — EXACT!")
                                    elif diff_ttc < 3:
                                        st.warning(f"⚠️ Total TTC ticket: {ticket_ttc:.2f}€ — Calcul: {calc_ttc:.2f}€ — Ecart: {diff_ttc:.2f}€")
                                    else:
                                        st.error(f"❌ Total TTC ticket: {ticket_ttc:.2f}€ — Calcul: {calc_ttc:.2f}€ — Ecart: {diff_ttc:.2f}€")
                                if ticket_ht:
                                    diff_ht = abs(calc_ht - ticket_ht)
                                    if diff_ht < 0.10:
                                        st.success(f"✅ Total HT ticket: {ticket_ht:.2f}€ — Calcul: {calc_ht:.2f}€ — EXACT!")
                                    elif diff_ht < 3:
                                        st.warning(f"⚠️ Total HT ticket: {ticket_ht:.2f}€ — Calcul: {calc_ht:.2f}€ — Ecart: {diff_ht:.2f}€")
        ...
        with sc_r:
            ...
            if "scan_articles" in st.session_state and st.session_state["scan_articles"]:
                ...
                st.caption(f"{len(articles)} articles — HT: {calc_ht:.2f}€ — TTC: {calc_ttc:.2f}€")
