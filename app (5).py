...
# ── Totaux : priorité aux vrais totaux du ticket fournisseur ──
# On utilise scan_totaux dès qu'il est présent (scan effectué),
# que les articles aient été ajoutés ou non.
_scan_totaux = st.session_state.get("scan_totaux", {})

if _scan_totaux and _scan_totaux.get("total_ttc"):
    g_ttc = float(_scan_totaux["total_ttc"])
    _ht   = _scan_totaux.get("total_ht")
    _tva  = _scan_totaux.get("total_tva")
    if _ht and _tva:
        g_ht  = float(_ht)
        g_tva = float(_tva)
    elif _ht:
        g_ht  = float(_ht)
        g_tva = round(g_ttc - g_ht, 2)
    elif _tva:
        g_tva = float(_tva)
        g_ht  = round(g_ttc - g_tva, 2)
    else:
        # Estimation depuis TTC (taux moyen ~20%)
        g_tva = round(g_ttc - g_ttc / 1.2, 2)
        g_ht  = round(g_ttc - g_tva, 2)
else:
    # Saisie manuelle ou pas encore de scan → calcul depuis les lignes
    tva_groups = defaultdict(float)
    for l in all_comp:
        tva_groups[l["tva"]] += l["tot_ht"]
